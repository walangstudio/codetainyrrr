...
pub async fn run(_reconcile: bool, daemon: bool) -> Result<()> {
    ...
    let coding_cli =
        std::env::var("CODING_CLI").unwrap_or_else(|_| cfg.catalog.project.default_cli.clone());
...
fn print_summary_banner(summary: &crate::installer::orchestrator::InstallSummary) {
    ...
    println!(
        "  {}",
        style(format!("── Install summary: {n_ok} ok, {n_fail} failed ──")).bold()
    );
    ...
    if !summary.failed.is_empty() {
        ...
        println!("    or run `codetainyrrr reconfigure` to retry / pick different ones.");
        println!();
    }
}
...
#[allow(dead_code)]
async fn _install_one_with_spinner(
    kind_label: &str,
    key: &str,
    spec: &str,
    kind: crate::installer::registry::Kind,
) -> Result<()> {
